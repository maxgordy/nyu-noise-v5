...
buffer_100ft <- st_buffer(nyu_properties, dist = 100) %>%
  st_transform(crs = st_crs(nyu_properties))
...
plot(st_geometry(nyu_properties), add = TRUE, col = "purple", pch = 16)
plot(st_geometry(data_within_buffer), add = TRUE, col = "pink", pch = 16)

st_crs(buffer)
nyu_properties <- nyu_properties %>% st_transform(4326)

#remove properties outside of manhattan
halls_within_buffer <- nyu_properties %>%
  filter(st_within(geometry, buffer, sparse = FALSE) %>% rowSums() > 0)
...
heatmap <- ggplot() +
  ...
  geom_sf(data = nyu_properties, aes(color = "NYU Properties"), fill = NA, alpha = 1, size = 2) +
