...
# Define the NYU residence halls and their addresses
nyu_residence_halls <- data.frame(
  building = c(
    "Brittany Hall", "Founders Hall", "Lipton Hall", 
    "Res College at Paulson", "Rubin Hall", "Weinstein Hall", 
    "Alumni Hall", "Broome Street", "Carlyle Court", "Coral Tower", 
    "Gramercy Green", "Greenwich Hall", "Lafayette Hall", 
    "Palladium Hall", "Second Street", "Seventh Street", 
    "Sixth Street", "Third North", "University Hall", "Clark Street"
  ),
  address = c(
    ...
    "110 East 14th Street", "55 Clark Street"
  )
)
...
head(mappluto_clean$address)

# Normalize addresses in both datasets
nyu_residence_halls <- nyu_residence_halls %>%
  mutate(address = str_to_upper(address))  # Convert to uppercase
...
nyu_properties <- mappluto_clean %>%
  filter(address %in% nyu_residence_halls$address)
